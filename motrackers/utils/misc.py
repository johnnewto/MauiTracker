...
def puttext(img, text, pos=None, fontscale=1.0, textcolor=(255,255,255), backcolor=(0,0,0)):
    (text_width, text_height) = cv.getTextSize(text, cv.FONT_HERSHEY_SIMPLEX, fontscale, 1)[0]
    if pos is None:
        pos = (img.shape[1]//2-text_width//2, img.shape[0]//2+text_height//2)

    rs_point = (pos[0]-2, pos[1]+2)
    re_point = (pos[0]+text_width+4, pos[1]-text_height-4)
    img = cv.rectangle(img, rs_point, re_point, backcolor, -1)
    return cv.putText(img, text, pos, cv.FONT_HERSHEY_SIMPLEX, fontscale, textcolor, 1, cv.LINE_AA)

def draw_tracks(image, tracks, dotsize=3, display_scale=1, colors=None):
    """
    Draw on input image.
    ...
    Args:
        ...
        scale float: display image can be at a different scale to tracking image
    ...
    try:
        ...
        for (objectID, trk) in tracks.items():
            trk_id = objectID
            xcentroid, ycentroid = int(trk.centroid[0] * display_scale), int(trk.centroid[1] * display_scale)
            # xcentroid, ycentroid = centroid[0], centroid[1]
            color = None
            puttext(image, f'{trk_id}', (xcentroid-10, ycentroid-15), fontscale=0.3, backcolor=color)
            # cv.putText(image, text, (xcentroid - 10, ycentroid - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
            if dotsize > 0:
                cv.circle(image, (xcentroid, ycentroid), dotsize, (0, 255, 0), -1)

    except:
        for d in tracks:
            color = colors[1]
            color = colors[int(d[4])%len(colors)]
    ...
    return image
        ...
        # for trk in tracks:
        #     xmin = int(trk[2]*display_scale)
        #     ymin = int(trk[3]*display_scale)
        #     width = int(trk[4]*display_scale)
        #     height = int(trk[5]*display_scale)
        #     class_id = int(trk[7])
        #
